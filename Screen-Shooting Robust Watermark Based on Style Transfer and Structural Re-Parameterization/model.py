...
from module import RepConv2d, MSCASpatialAttention, HWCBlock
...
class DoubleConv(nn.Module):
    def __init__(self, inchannel, outchannel):
        ...
        # 重参数化
        self.conv = nn.Sequential(
            RepConv2d(inchannel, outchannel, 3, 1, 1),
            RepConv2d(outchannel, outchannel, 3, 1, 1),
        )
...
class U_Net_Encoder_Diffusion(nn.Module):
    ...
    def forward(self, x, watermark):
        # matrix = np.random.randint(0, 2, size=(1, 30))
        # # 转换为PyTorch的Tensor
        # watermark = torch.tensor(matrix, dtype=torch.float)
...
class Encoder_Decoder(nn.Module):
    ...
    def forward(self, x, m, stylenet):


        Encoded_image = self.Encoder(x, m)

        Noised_image = self.Noiser(Encoded_image, stylenet)
