...
from distoration_model import model as style_net
import torch.nn.functional as F


def load(dir_chck, netG_a2b, epoch=[]):
    if not epoch:
...
class Solver(object):
    ...
    def test_embedding(self):
        # load style_gan
        netG_a2b = style_net.ResNet(3, 3, 64, 'inorm', nblk=6)
        init_net(netG_a2b, init_type='normal', init_gain=0.02, gpu_ids=0)
        netG_a2b = load('./distoration_model/model/', netG_a2b)
        netG_a2b.to('cuda')
        ...
        for i, (data, m, num) in enumerate(data_loader):
            ...
            Encoded_image, Noised_image, Decoded_message = self.net_ED(inputs, m, netG_a2b)
            ...
            for j in range(Encoded_image.shape[0]):
                ...
                cv2.imwrite('./Datasets/qianru/' + str(index) + '.png', I2)
    ...
    def train_mask(self):
        ...
        for epoch in range(start_epoch, self.num_epoch):
            ...
            for i, (data, m, v_mask) in enumerate(data_loader):
                ...
                inputs.requires_grad = True
                Encoded_image, Noised_image, Decoded_message = self.net(inputs, m, netG_a2b)
                loss_de = criterion_MSE(Decoded_message, m)
                ...
                # self.optimizer_Discriminator.zero_grad()
                # i_generate, i_local, i_glob = self.net_Discriminator(inputs.detach())
                # # i_discriminate_loss = infonce_loss(i_local, i_glob)
                # # i_discriminate_loss.backward()
                # i_generate_loss = criterion(i_generate, d_label_host)
                #
                # #i_loss.backward()
                # # i_generate_loss.backward()
                #
                # #encoded_images
                # e_generate, e_local, e_glob = self.net_Discriminator(Encoded_image.detach())
                # # e_discriminate_loss = infonce_loss(e_local, e_glob)
                # # e_discriminate_loss.backward()
                # e_generate_loss = criterion(e_generate, d_label_decoded)
                # d_loss = i_generate_loss + e_generate_loss
                # d_loss.backward()
                # # e_generate_loss.backward()
                #
                # self.optimizer_Discriminator.step()
                ...
                loss_denoise = criterion_MSE(Encoded_image * mask.float(), inputs * mask.float()) * 2 + criterion_MSE(
                    Encoded_image * v_mask.float(), inputs * v_mask.float()) * 0.02
                loss = loss_message * self.lambda1 + loss_denoise * self.lambda2 + g_loss * self.lambda3

                # loss_denoise = criterion_MSE(Encoded_image * mask.float(), inputs * mask.float()) * 2 + criterion_MSE(
                #     Encoded_image * v_mask.float(), inputs * v_mask.float()) * 0.5
                # loss = loss_message * self.lambda1 + loss_denoise * self.lambda2 + g_loss * self.lambda3
                ...
                # loss_message = criterion_MSE(Decoded_message, m)
                # loss_denoise = criterion_MSE(Encoded_image * mask.float(), inputs * mask.float()) * 2 + criterion_MSE(
                #     Encoded_image * v_mask.float(), inputs * v_mask.float()) * 0.1
                # loss = loss_message * self.lambda1 + loss_denoise * self.lambda2 + g_loss * self.lambda3
                # self.net_optimizer.zero_grad()
                # loss.backward()
                #
