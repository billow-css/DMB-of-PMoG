...
本文档定量描述联合噪声演示脚本 `AS_VALs/demos/python/demo_combined_with_pimog.py`：

将**非线性运动模糊**与 **PIMoG `ScreenShooting`** 串联，并用抖动强度**反比约束**摩尔纹权重。
运动模糊的完整推导见 [`运动模糊噪声层.md`](运动模糊噪声层.md)。
...
其中 $w_k=1/K$ 为均匀快门权重，$\mathcal{B}$ 为双线性采样。细节见 [`运动模糊噪声层.md`](运动模糊噪声层.md)。
...
| `path_power` | $2$ | 进度幂 $\lambda=t^p$（匀加速） |
...
| 模块 | 文档 / 代码 | 在本脚本中的角色 |
|:---|:---|:---|
| 非线性运动模糊 | `运动模糊噪声层.md` | 产生 $B$ 与 $m(u,v)$ |
